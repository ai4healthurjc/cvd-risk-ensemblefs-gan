...
def mrae_hugo(y_real: np.array, y_pred: np.array) -> float:
...
def compute_mae(y_real: np.ndarray, y_pred: np.ndarray):
...
def compute_mse(y_real: np.ndarray, y_pred: np.ndarray):
    return np.mean(np.square(_error(y_real, y_pred)))


def _error(y_real: np.ndarray, y_pred: np.ndarray):
